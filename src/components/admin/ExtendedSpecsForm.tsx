  ...
  const [values, setValues] = useState<PhoneExtendedSpecsFormValues>(() => {
    ...
    for (const group of EXTENDED_SPEC_GROUPS) {
      for (const field of group.fields) {
        out[field.key] =
          (initialValues?.[field.key as keyof PhoneExtendedSpecs] as
            | string
            | null) ?? "";
      }
    }
  ...
  return (
    <div className="space-y-6">
      <p className="text-xs text-ink/50">
        Leave any row blank and it simply won&apos;t appear on the public spec table.
        Fields scroll internally if content runs long — no need to keep them
        expanded.
      </p>
