...
export function FeaturedSectionEditor({
  ...
  async function selectForSlot(i: number, slug: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setSlotError(
          "Could not load that phone — it may have been deleted. Please try another.",
        );
        return;
      }
      ...
      setSlotError(
        "Something went wrong loading that phone. Please try again.",
      );
    }
  }
  ...
  async function handleSave() {
    ...
    try {
      const ids = current
        .filter((p): p is SlotPhone => !!p && p.isPinned)
        .map((p) => p.id);
    ...
    <div className="mb-8 rounded-lg border border-border bg-white p-4">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
        {current.map((slot, i) => (
          <div
            key={i}
            className="rounded-md border border-dashed border-border p-2 text-center"
          >
            {slot ? (
              <>
                <span
      ...
      {slotError && <p className="mt-2 text-xs text-red-600">{slotError}</p>}
      ...
      {modalIndex !== null && (
        <SwapModal
          ...
          onSelect={(slug) => selectForSlot(modalIndex, slug)}
        />
      )}
    </div>
  );
