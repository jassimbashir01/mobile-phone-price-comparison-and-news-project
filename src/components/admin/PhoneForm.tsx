...
import { createPhone, updatePhone, savePhoneImages } from '@/lib/actions/phones';
...
export function PhoneForm({ phone, brands }: { phone?: PhoneWithDetails; brands: Brand[] }) {
  ...
  const {
    ...
    defaultValues: phone
      ? {
          ...
          price_usd: phone.price_usd ?? undefined,
      ...
      : { status: 'available', is_featured: false, is_sponsored: false, sort_order: 0 },
  });
  ...
  async function onSubmit(values: PhoneFormValues) {
    ...
    try {
      if (phone) {
        ...
        // Images need a phone_id to attach to, so a brand-new phone goes
        // straight to its own edit page where the uploader becomes available.
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="max-w-3xl space-y-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field id="brand_id" label="Brand" error={errors.brand_id?.message}>
            <select id="brand_id" {...register('brand_id')} className={inputClass}>
          ...
          <Field id="price_pkr" label="Price (PKR)">
            <input id="price_pkr" type="number" {...register('price_pkr')} className={inputClass} />
          </Field>
          <Field id="price_usd" label="Price (USD)">
            <input id="price_usd" type="number" step="0.01" {...register('price_usd')} className={inputClass} />
          </Field>
          <Field id="sort_order" label="Sort Order">
            <input id="sort_order" type="number" {...register('sort_order')} className={inputClass} />
          </Field>
        </div>
        ...
        <div className="flex gap-6">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" {...register('is_sponsored')} /> Sponsored
          </label>
        </div>

        <Field id="seo_description" label="SEO Description">
          <textarea id="seo_description" rows={3} {...register('seo_description')} className={inputClass} />
        </Field>
        ...
        <fieldset className="rounded-lg border border-border p-4">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field id="network_type" label="Network Type">
              <select id="network_type" {...register('network_type')} className={inputClass}>
                <option value="">—</option>
            ...
            <Field id="ram_gb" label="RAM (GB)"><input id="ram_gb" type="number" {...register('ram_gb')} className={inputClass} /></Field>
            <Field id="storage_gb" label="Storage (GB)"><input id="storage_gb" type="number" {...register('storage_gb')} className={inputClass} /></Field>
            <Field id="display_size" label="Display Size (inches)"><input id="display_size" type="number" step="0.1" {...register('display_size')} className={inputClass} /></Field>
            <Field id="display_type" label="Display Type"><input id="display_type" {...register('display_type')} className={inputClass} /></Field>
            <Field id="main_camera_mp" label="Main Camera (MP)"><input id="main_camera_mp" type="number" step="0.1" {...register('main_camera_mp')} className={inputClass} /></Field>
            <Field id="battery_mah" label="Battery (mAh)"><input id="battery_mah" type="number" {...register('battery_mah')} className={inputClass} /></Field>
            <Field id="processor" label="Processor"><input id="processor" {...register('processor')} className={inputClass} /></Field>
          </div>
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
            {(['bluetooth', 'wifi', 'dual_sim', 'fm_radio', 'memory_card', 'mp3', 'video_recording', 'has_camera'] as const).map((field) => (
              <label key={field} className="flex items-center gap-2 text-sm capitalize">
                <input type="checkbox" {...register(field)} /> {field.replace('_', ' ')}
              </label>
            ))}
          </div>
        </fieldset>
        ...
        >
          {isSubmitting ? 'Saving…' : phone ? 'Save Changes' : 'Create Phone & Continue to Images'}
        </button>
      </form>
