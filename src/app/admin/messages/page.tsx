...
export default async function AdminMessagesPage() {
  const [messages, profile] = await Promise.all([getAllContactMessagesAdmin(), getCurrentUserProfile()]);
  const unreadCount = messages.filter((m) => !m.is_read).length;

  return (
    <div>
      <h1 className="mb-4 text-xl font-bold">
        Messages {unreadCount > 0 && <span className="text-primary">({unreadCount} unread)</span>}
      </h1>
      {messages.length === 0 ? (
