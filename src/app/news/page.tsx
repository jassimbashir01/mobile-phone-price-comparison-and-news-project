...
export async function generateMetadata({
  ...
  return {
    ...
    description: 'Latest mobile phone news, launches, and updates from Pakistan and around the world.',
...
export default async function NewsListingPage({
  ...
  return (
    <PageShell>
      ...
      <h1 className="mb-4 text-xl font-bold">Mobile Phone News</h1>

      <div className="mb-5 flex flex-wrap gap-2">
        <Link
          href="/news"
          className={`rounded-full border px-3 py-1 text-xs ${
            !brandSlug ? 'border-primary bg-primary text-white' : 'border-border bg-white'
          }`}
        >
          All
        </Link>
        {brands.map((b) => (
          <Link
            ...
            className={`rounded-full border px-3 py-1 text-xs ${
              brandSlug === b.slug ? 'border-primary bg-primary text-white' : 'border-border bg-white'
            }`}
          >
            {b.name}
          </Link>
